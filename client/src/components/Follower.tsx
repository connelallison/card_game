...
class Follower extends Card {
  ...
  categories() {
    return this.props.object && this.props.object.categories
      ? <div className='followerCategories'>{this.props.object.categories.map(category => <span key={`${this.props.object.objectID}:${category}`} className='followerCategory'>{category}</span>)}</div>
      : null
  }
