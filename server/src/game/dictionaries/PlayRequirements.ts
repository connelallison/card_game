...
const PlayRequirements: {[index: string]: PlayRequirementFactory} = {
    minFriendlyFollowers: (object: GameObject, values: {min: DynamicNumber}) => () => (object.controller().board.length >= values.min()),
    minEnemyFollowers: (object: GameObject, values: {min: DynamicNumber}) => () => (object.controller().opponent.board.length >= values.min()),
    minAllFollowers: (object: GameObject, values: {min: DynamicNumber}) => () => (object.controller().board.length + object.controller().opponent.board.length >= values.min()),
