...
abstract class Card extends GameObject {
  ...
  tooltipsReport(localisation: LocalisationString = 'english'): LocalisedNameAndText[] {
    ...
    if (this['categories']?.Barbarian || this.tooltips.includes('barbarian')) rawTooltips.push(Tooltips.barbarian)
    if (this['categories']?.Legend || this.tooltips.includes('legend')) rawTooltips.push(Tooltips.legend)
    if (this['categories']?.Noble || this.tooltips.includes('noble')) rawTooltips.push(Tooltips.noble)
    if (this['categories']?.Tech || this.tooltips.includes('tech')) rawTooltips.push(Tooltips.tech)
    if (this['categories']?.Underclass || this.tooltips.includes('underclass')) rawTooltips.push(Tooltips.underclass)
    if (this['categories']?.Woman || this.tooltips.includes('woman')) rawTooltips.push(Tooltips.woman)
